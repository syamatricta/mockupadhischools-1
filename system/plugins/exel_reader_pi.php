...
/*		echo $data->sheets[0]['numRows'];die();
		...
		if($data->sheets[0]['numRows']==0){
			$ci->session->set_flashdata('msg', 'Xls is not in appropriate format8');
			...
			echo "<br/>".count($data->sheets[0]['cells']);
			echo "<br/>".end(array_keys($data->sheets[0]['cells']));
			echo "<br/>";
			print_r($data->sheets[0]['cells']);
			echo '</pre>';exit;*/
			$endRowNo	= end(array_keys($data->sheets[0]['cells']));
			for ($i = 1; $i <= $endRowNo; $i++) {
				for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
					if($data->sheets[0]['numCols'] < 400){
						...
						if($i==1 || $i==$m+6) {
							...
							$No=$k+1;
							//echo $No.')'.$newData.'<br>';
							if(empty($newData) && '0' != $newData && $j==1) {
								
								$error=1;
								$a = "Error in Question Nos (".var_dump($newData, true).")";
								$error_repeat = $No;
								$error_que .= $No."[".$newData.']'.',';
								...
								if($i==11){
									if($invalid >= 2){
										$ci->session->set_flashdata('msg', 'Xls is not in appropriate format9');
										if($type == 'quiz'){
						...
						return 'Inappropriate Format: Number of columns is not less than 4. Some row contain '. $data->sheets[0]['numCols'].' columns data'.$newData;
					}
				}
			} 
		}
			...
			for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
				...
				if($i==1 || $i==$m+6){
					if(!empty($newData) && $j==1){ 
                        ...
                        /* $error_style	= '';
						if($i>1 && 0 != ($i-1)%6){
							$error_style	= '<span style="color:red">ERROR</span>';
						}
						if($i > 1 && ($prevno+6) != $i){
							$error_style	= '<span style="color:red">ERROR</span>';
						}
                        $prevno = $i;
						echo $i.')'.$newData.$error_style.'<br/>';*/
...
/*		echo $data->sheets[0]['numRows'];die();
		...
		$k=0; 
		if($data->sheets[0]['numRows']==0){		
			$ci->session->set_flashdata('msg', 'Xls is not in appropriate format1');	//errrorrrr							
			...
			for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {			
				...
				for ($j = 1; $j <= 1; $j++) {
					...
					if($invalid>=1){
						$ci->session->set_flashdata('msg', 'Xls is not in appropriate format2');
						redirect ('dictionary/upload');
					}					
...
/*		if(count($data->sheets[0]['numRows']))
		...
		if($data->sheets[0]['numRows'] == 0 || $data->sheets[0]['numCols'] == 0){		
			...
			//redirect ($ci->config->item ('upload_exam'));
			$err_msg = 'Xls is not in appropriate format3';
			return $err_msg;
		}else{	
			for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {		
				for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
					...
					if($i==1 || $i==$m+6){												
						if(empty($newData) && '0' != $newData && $j==1){
							...
							if($i==13){
								if($invalid>=2){
									//$ci->session->set_flashdata('msg', 'Xls is not in appropriate format');								
									//redirect ($ci->config->item ('upload_exam'));
									$err_msg = 'Xls is not in appropriate format4';
									return $err_msg;
								}
							}							
						}
						...
						if(is_array($arr_answers) && count($arr_answers) == 4) {
							...
							if(($answers_arr['Y'] != 1) || ($answers_arr['N'] != 3)) {
								$err_msg = 'Xls is not in appropriate format5';
								return $err_msg;
							}
						} else {
							$err_msg = 'Xls is not in appropriate format6';
							return $err_msg;
						}
		...
		if($error)
			//return trim($error_data,', ');
			return 'Xls is not in appropriate format7';
		else 	
			return false;
