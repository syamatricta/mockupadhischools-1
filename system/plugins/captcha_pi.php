...
//		$pool = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
	...
	// -----------------------------------
        ...
        if($data['admin']){
            $bg_color		= imagecolorallocate ($im, 180, 231, 248);
            $border_color	= imagecolorallocate ($im, 255, 255, 255);
            $text_color		= imagecolorallocate ($im, 0, 0, 0);
            $grid_color		= imagecolorallocate($im, 180, 221, 248);
            $shadow_color	= imagecolorallocate($im, 0, 0, 0);
        }else{
            $bg_color		= imagecolorallocate ($im, 156,151,158);
            $border_color	= imagecolorallocate ($im, 156,151,158);
            $text_color		= imagecolorallocate ($im, 255, 255, 255);
            $grid_color		= imagecolorallocate($im, 156,151,158);
            $shadow_color	= imagecolorallocate($im, 0, 0, 0);
        }
		...
		if ($use_font == FALSE)
		{
			$y = rand(0 , $img_height/2);
                        if($data['admin']){
                            ...
                            imagestring($im, $font_size, $x, $y, strtoupper(substr($word, $i, 1)), $text_color);
                        }
		...
		{		
			$y = rand($img_height/2, $img_height-3);
                        if($data['admin']){
                            ...
                            imagettftext($im, $font_size, $angle, $x, $y, $text_color, $font_path, strtoupper(substr($word, $i, 1)));
                        }
			$x += $font_size;
		}
	}
