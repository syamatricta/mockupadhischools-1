	...
	class Authentication
	{
		...
		function process_user_ajax_login ($login = NULL, $request = '')
		{
		    ...
		    */
			...
			$this->CI->db->select ("id AS USERID, firstname AS FIRST_NAME, lastname as LAST_NAME, emailid AS USERNAME, 
                            emailid AS EMAIL,status as STATUS, driving_license AS DRIVING_LICENSE");
			...
			if (0 < $select_query->num_rows ()){
				...
				$row 			= $select_query->row();   
                                ...
                                $this->CI->db->select ("enrolled_date as CREATED_AT");
                                $this->CI->db->where ('userid', $row->USERID);
                                $this->CI->db->order_by ('id', "ASC");
                                $enroll_query    = $this->CI->db->get ('adhi_user_course');
                                
                                if (0 < $enroll_query->num_rows ()){
                                    $enroll_row  = $enroll_query->row();
                                    $enroll_created = $enroll_row->CREATED_AT;
                                }else{
                                    $enroll_created = "0000-00-00";
                                }
				...
				if($this->checkUserUniqueLogin($row->USERID,$row->EMAIL,$login['forced_login'])){
                                    ...
                                    if($this->checkUserExamMode($row->USERID,$login['forced_login'])){
                                        ...
                                        $session_data 	= array (
		                                           	...
		                                           	'STATUS' 	=> $row->STATUS,
		                                           	'USERTYPE' 	=> 'N',
                                                                'USER_CREATED_AT' 	=>	("0000-00-00" != $enroll_created) ? date("Y-m-d",strtotime($enroll_created)) : date('Y-m-d', strtotime('-1 day', strtotime($this->CI->config->item ('cut_off_date'))))
		                              	        );
						...
						$response['msg']="Already logged in, please login forcefully";	
                            ...
                            if(0 == $user->status){
                                ...
                                $response['error_status']   = 'trial_period_expired';
                                $response['msg']            = 'Your Trial account is expired.';
                            }else if(4 == $user->status){
                                $response['status']         = 'error';
                                $response['userid']         =  $user->id;
                                $response['error_status']   = 'trial_account_blocked';
                                $response['msg']            = 'Your account has been blocked, please contact administrator.';
                            }
			}else {
                                $response['user_type']          = '';
				$response['status']="error";
