...
<?php 	foreach($courses_m as $courses_m){ ?>
			<div  class="filedforrate"  >
				...
				<img  src="<?php  echo ssl_url_img();?>course_checkbox_uncheck.png" width="13" height="13" id="course_chkimg<?php echo $courses_m['id']; ?>" onClick="javascript:showterms(<?php echo $courses_m['id']; ?>), checkrate();"/>&nbsp;&nbsp;
                                <input type="checkbox" <?php if($courses_m['child_cnt'] ==0){?> class="scheck display-none" <?php }else{ ?>  DISABLED <?php }?> name="course[]"   id="course<?php if($courses_m['child_cnt'] ==0) echo $courses_m['id']; else echo 0; ?>" value="<?php if($courses_m['child_cnt'] ==0)  echo $courses_m['id']; else echo 0; ?>" onClick="javascript:showterms(this.value), checkrate();"  />
					<span onClick="javascript:showterms(<?php echo $courses_m['id']; ?>), checkrate();"><?php 	if($courses_m['child_cnt'] ==0){
					...
					<?php if($courses_m['child_cnt'] ==0) ?>
                                <input type="hidden" name="selagree<?php echo $courses_m['id']; ?>" id="selagree<?php echo $courses_m['id']; ?>" value="0" />
...
<!--					<div class="agreementbox">
					...
					<div  class="filedforterm agreement" >
						<div class="agreementagreecheck"><img  src="<?php  echo ssl_url_img();?>checkbox_uncheck.png" width="25" height="24" id="agree<? echo $courses_m['id'];?>" onClick="javascript:showcheck_addnewcourse(<? echo $courses_m['id'];?>), checkrate();"/><!--<input type="checkbox" name="agree<?php echo $courses_m['id']; ?>" id="agree<?php echo $courses_m['id']; ?>" value="<?php echo $courses_m['id']; ?>" onClick="javascript:showcheck_addnewcourse(this.value),checkrate();">--></div>
						<div class="agreementagreetext" onClick="javascript:showcheck_addnewcourse(<? echo $courses_m['id'];?>), checkrate();">I Agree  </div>
						<div class="floatleft"><img  src="<?php  echo ssl_url_img();?>checkbox_uncheck.png" width="25" height="24" id="disagree<?php echo $courses_m['id']; ?>"   onClick="javascript:showuncheck(<? echo $courses_m['id'];?>), checkrate();"/><!--<input type="checkbox" name="disagree<?php echo $courses_m['id']; ?>" id="disagree<?php echo $courses_m['id']; ?>" value="<?php echo $courses_m['id']; ?>" onClick="javascript:showuncheck(this.value),checkrate();"  />--> </div>
						<div class="agreementdonttext" onClick="javascript:showuncheck(<? echo $courses_m['id'];?>), checkrate();">I Don't Agree </div>
					</div>
                                    </div>
				</div>
				<div class="clearboth"></div> 
