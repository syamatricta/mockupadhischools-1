	...
	function confirm_password()
	{
		...
		function exam_start() 
		{
			...
			if($data[0]['exam_status']!='D'){ 
				...
				if(!$this->authentication->logged_in("normal")){
					redirect('exam/exam_error/2');
				}
				
				if(isset($_POST) && $this->session->userdata ('ANSWERID')){
					redirect('exam/exam_error/1');
				}
				...
				if(!$time_end_status){
					...
					$userid = $this->session->userdata ('USERID');
					$exam_mode = $this->user_exam_model->already_exam_mode($userid,0);
					...
					if($usercourseid){
						...
						$this->user_exam_model->change_effective_date($userid,$usercourseid,'samecourse');
                                                ...
                                                /*$this->user_exam_model->changeUserCourseEffectiveDates($userid, $usercourseid);*/
