...
class Trial_quiz extends Controller {
	...
	function Trial_quiz() {
		...
		$this->load->model ( array('admin_trial_account_model', 'trial_account_model') );
		...
		$this->gen_contents ['js'] = array ('tooltips.js', 'userdetails.js', 'popcalendar.js', 'quiz_user.js', 'effects.js', 'scriptaculous.js', 'modalbox.js' );
                $this->gen_contents['selected_nav']	=   'quiz';
                
                $this->settings                   = $this->admin_trial_account_model->getSettings();
	...
	function quizlist() {
		...
		if ($this->authentication->logged_in ( "trial" )) {
                    ...
                    $this->gen_contents ["course_id"]   = $quiz_details->course_id;
                    $edition                            = $quiz_details->edition;

                    $this->gen_contents ['listStatus']  = ( int ) $this->quiz_model->checkQuizStatus ( $this->gen_contents ["course_id"], $edition );
                    $this->gen_contents ['quizlist']    = $this->trial_account_model->getQuizList ($settings->quiz_list_id);

                    $this->template->set_template('user');
                    $this->template->write_view('content', 'reskin/trial/quiz/quizlist', $this->gen_contents);
	...
	function quizrule() {
		...
		if ($this->authentication->logged_in ( "trial" )) {
			...
			$this->gen_contents ["quiz_id"] = $this->uri->segment ( 3 );
                        if($this->settings->quiz_list_id != $this->gen_contents ["quiz_id"]){
                            $this->gen_contents ["message"] = "Unautherised access to this quiz";
                            $this->template->set_template('user');
                            $this->template->write_view('content', 'reskin/trial/quiz/error_quiz', $this->gen_contents);
                            $this->template->render();
                        }else{
                            ...
                            if (@$quiz_course [0]->quiz_status == 'E') {
                                ...
                                $this->template->set_template('user');
                                $this->template->write_view('content', 'reskin/trial/quiz/quiz_rule', $this->gen_contents);
                                $this->template->render();
                            } else {
	...
	{ 
		...
		if (! $this->authentication->logged_in ( "trial" )) 
		{
			redirect ( 'trial_quiz/quiz_error/2' );
		}
		...
		if (isset ( $_SERVER ['HTTP_REFERER'] ) && $_SERVER ['HTTP_REFERER']) 
		{
			...
			//$this->gen_contents ['quiz_list'] = $this->quiz_model->get_quizlist ( $course_id, $this->session->userdata ( 'USERID' ),$edition );
                        $this->gen_contents ['quiz_list'] = $this->trial_account_model->getQuizList ($settings->quiz_list_id);
			...
			if($this->gen_contents["quiz_id"]) 
			{
				...
				redirect ( 'trial_quiz/quiz_error/3' );
			}
		} else {
			redirect ( 'trial_quiz/quiz_error/1' );
		}
	...
	function quiz_examination(){
		...
		if(!$this->authentication->logged_in("trial")){
			redirect('trial_quiz/exam_error/2');
		}
		if(($this->session->userdata('start') || $_POST) && $this->session->userdata('end')){
			...
			if($que_no<=count($sess_ques)){
				...
				$this->load->view ( "reskin/trial/quiz/quiz_header", $this->gen_contents );
				$this->load->view ( 'reskin/trial/quiz/quiz_page' );
				$this->load->view ( "reskin/trial/quiz/quiz_footer" );
			
			}
			else 
				redirect('trial_quiz/quiz_end');
	...
	function quiz_end() {
		...
		$config ['uri_segment'] = 3;
                $config['num_links'] = 1;
		$config ['base_url'] = base_url () . 'index.php/trial_quiz/quiz_end/';
		...
		$this->load->view ( "reskin/trial/quiz/quiz_header", $this->gen_contents );
		$this->load->view ( 'reskin/trial/quiz/quiz_end', $this->gen_contents );
		$this->load->view ( "reskin/trial/quiz/quiz_footer" );
	...
	function quiz_disable() {
		...
		$this->load->view ( "reskin/trial/quiz/quiz_header", $this->gen_contents );
		$this->load->view ( 'reskin/trial/quiz/quiz_disable', $this->gen_contents );
		$this->load->view ( "reskin/trial/quiz/quiz_footer" );
	}
	...
	function change_quizmode() {
		...
		if ($data->quiz_status != 'D') {
			...
			if (isset ( $reset ) && (empty ( $reset ))) {
				...
				//echo $mode_id;		
                                $mode_id    = $this->session->userdata ( 'TRIAL_USERID' );
	...
	function quiz_error() {
		...
		$this->load->view ( "reskin/trial/quiz/quiz_header", $this->gen_contents );
		$this->load->view ( 'reskin/trial/quiz/quiz_reuse', $this->gen_contents );
		$this->load->view ( "reskin/trial/quiz/quiz_footer" );
	}
	...
	function change_quiz() {
		...
		redirect ( 'trial_quiz/quiz_start' );
