	...
	class Admin_user extends Controller
	{
		...
		function list_user_details ()
		{
			...
			if($this->gen_contents["course_completed"]){
				...
				$this->gen_contents["userdetails"]	=	$this->admin_user_model->select_userdetails_completed($config['per_page'],$this->uri->segment(3),$this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"], $this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"]);
				$config['total_rows']   		= 	$this->admin_user_model->qry_count_userdetails_completed($this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"],$this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"]);

			}else{

				$this->gen_contents["userdetails"]	=	$this->admin_user_model->select_userdetails($config['per_page'],$this->uri->segment(3),$this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"], $this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"]);
				$config['total_rows']   		= 	$this->admin_user_model->qry_count_userdetails($this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"],$this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"]);
			}
...
exit();*/
        ...
        function edit_otp_user()
	{
        ...
        function _profile_progress($user_id, $email_id){
            ...
            if($user_stat['completed_all_exams'] && $this->course_model->isCrashCourseUser($email_id)) {
                ...
                if($profile_progress && $this->_item_key_exists($profile_progress, 'state_exam_applied')){
                    ...
                    $user_stat['state_exam_applied']    = array('class' => 'active visited');
                    ...
                    if($profile_progress && $this->_item_key_exists($profile_progress, 'obtained_license')){
                        ...
                        $user_stat['obtained_license']      = array('class' => 'active visited');
                    }
                }
            }
        ...
        function adhi_sale_list(){
            $this->load->model('admin_user_model');
            $this->admin_user_model->getAdhiSaleList();
        }
       ...
       function list_export_to_excel() {
           ...
           if(!empty($_POST)) {
            ...
            /*****sree 080710**/
            if($search_params["course_completed"]){
                $users  = $this->admin_user_model->getAllUsers($search_params, 'course_completed');
            }else{
                $users  = $this->admin_user_model->getAllUsers($search_params);
            }
