        ...
        function ajax_register() {
            ...
            if($this->form_validation->run() == TRUE) {
                ...
                if($check_in_trial){
                    ...
                    }else if(3 == $check_in_trial->status){                        
                        $this->session->set_userdata(array('EXPIRED_TRIAL_ID' => $check_in_trial->id, 'TRIAL_EXPIRED' => TRUE));
                        $this->gen_contents['errors']   = 'It looks like you already had the chance to check out our program. '
                                . 'Give us a call to sign up 888-768-5285. <br/><a href="'.base_url().'trial_account/upgrade"><h5><b>Click here to upgrade</b></h5></a>';
                    }else if(4 == $check_in_trial->status){
                        $this->gen_contents['errors']   = 'Your account has been blocked, please contact adminstrator.';
                    }
                }
                if($check_in_user){
                    ...
                    if($adhi_user = $this->trial_account_model->getAdhiUserByEmail($user->email)){
                        ...
                        if($today <= $check){
                            ...
                            $save_data = array(
                                ...
                                'will_expire_at'    => date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s', $today).' +'. $settings->validity_days .' days'))
                            ...
                            if($settings->validity_days > 1){
                                ...
                                $save_data['marketing_email_at'] = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s', $today).' +'. $maketing_day .' days'));
                            }
