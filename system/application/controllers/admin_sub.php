...
class Admin_sub extends Controller
{
    ...
    function list_subadmins()
    {
        ...
        $config['base_url'] 			= base_url().'admin_sub/list_subadmins/';
        $config['per_page'] 			= '10';
        $config['num_links']                    = 1;
	...
	$this->gen_contents["search_email"] = '';
	    ...
	    $this->gen_contents["search_permission"] = $this->Common_model->safe_html($this->input->post('txtSrchPermission'));
	    ...
	    $this->gen_contents["search_permission"] = $this->session->flashdata('search_permission');
	...
	$this->session->set_flashdata('search_permission',$this->gen_contents["search_permission"]);
        
        
        $this->gen_contents["subadmin_details"]	= $this->admin_subadmin_model->select_subadmindetails($config['per_page'],$this->uri->segment(3),$this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"], $this->gen_contents["search_permission"]);
	
        $config['total_rows']   		= $this->admin_subadmin_model->qry_count_subadmindetails($this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"], $this->gen_contents["search_permission"]);
    ...
    function _init_registration_rules()
    {
        ...
        $this->form_validation->set_rules('extra_permission', 'Extra Permission', 'trim');
    }
    ...
    function _init_subadmin_regdetails()
    {
        return $this->gen_contents['data'] = array(
                    ...
                    "user_type"         => '2',
                    "sub_admin_permission" => $this->Common_model->safe_html($this->input->post('extra_permission'))
                     );  
    }
