...
*/
// ------------------------------------------------------------------------ 
	class Career_event extends Controller
	{
		/**
		 * General contents
		 */
		...
		/**
		 * Career_event constructor
		 */
		
		function Career_event () {
			parent::Controller();
			...
			$this->gen_contents['title']		= 'Career Events';
			$this->gen_contents['page_title']	= 'Real Estate Career Events';
		...
		function index(){
                    $this->gen_contents['current_page']  = 'career_event';
                    ...
                    $this->template->write_view('content', 'reskin/career_event/schedule', $this->gen_contents);
		...
		function getClassforDay(){
		    ...
		    $this->gen_contents['date'] = $date;			 
                    ...
                    $this->gen_contents['arr_class'] = $this->admin_career_event_model->dbClasslistandlocation($date, $region_id, $subregion_id);
                    if($this->gen_contents['arr_class']){
                        $this->load->view ('reskin/career_event/eachday', $this->gen_contents);
                    }else {
                        echo '';
                    }
                ...
                function book_event(){
                    ...
                    $output = array('type' => 'error', 'message' => '');
                    ...
                    if($this->form_validation->run() == TRUE) {
                        ...
                        $event      = $this->admin_career_event_model->isEventExist($event_id);
                        if(!$event){
                            $output['message']  = 'Event not exist';
                        }else if($this->admin_career_event_model->hasBookedEarly($event_id, array('email' => $email))){
                            $output['message']  = 'You have already booked the event';
                        }else{
                            $save_data = array(
                            ...
                            if($this->admin_career_event_model->saveBooking($save_data)){
                                $output['type']     = 'success';
                                $output['message']  = 'Thank you for booking';
                                $mail_data          = array(
                                    'first_name'    => $this->input->post('first_name'),
                                    ...
                                    'event_date'    => formatDate(date('Y-m-d', strtotime($this->input->post('event_date')))),
                                    'start_time'    => date('h:i a', strtotime($event->time_start)),
                                    'end_time'      => date('h:i a', strtotime($event->time_end)),
                                    'location'      => $event->region_name.', '.$event->subregion_name.'<br/>'.$event->subregion_address,
                                    'description'   => $event->description,
                                    'parking_info'  => $event->parking_info
                                );
                                $sendmail           = $this->_email_to_user($this->input->post('email'), $mail_data, 'Career Seminar Confirmation', 'self_booking');
                            }else{
                                $output['message']  = 'Failed to book the event';
                            }
                ...
                function _email_to_user($to, $data, $subject, $type = 'register'){
                    $bcc_emails         = $this->config->item('career_seminar_email_bcc');
                    $contents		= 'Dear '.$data['first_name'].',<br/><br/> ';
                    $admin_bcc          = '';
                    if('self_booking' == $type){
                        $contents       .= '<p>Congrats on reserving your seat for our upcoming career seminar with ADHI Schools!</p>

                                            <p>Find the details below:</p>

                                            <p>Name : '.$data['name'].'<br/></p>
                                            <p>Email : '.$data['email'].'<br/></p>
                                            <p>Phone : '.$data['phone'].'<br/></p>

                                            <p><h3>Event Details</h3></p>
                                            <p><h4>'.$data['event_name'].'</h4></p>
                                            <p>'.$data['description'].'</p>
                                            <p>Date : '.$data['event_date'].'<br/></p>
                                            <p>Timings : '.$data['start_time'].' - '.$data['end_time'].'<br/></p>
                                            <p>Location : '.$data['location'].'<br/></p>
                                            <p>Parking Info : '.$data['parking_info'].'<br/></p>';
                        $admin_bcc          = 'yes';
                    }
		...
		function get_classes(){ 
                    ...
                    //$data["course_list"] = $this->admin_career_event_model->dbSelectAllCourses();
                    if($device=='large'){
                        $data["course_list"] = $this->admin_career_event_model->dbGetCoursesByDate($first_day_of_month,$last_day_of_month, $region_id, $subregion_id);
                    }else {
			$data["course_list"] = $this->admin_career_event_model->dbGetCoursesByDateMobile($first_day_of_month,$last_day_of_month, $region_id, $subregion_id);                        
                    }
		...
		function _date($time, $course='', $region='', $sub_region=''){
			...
			if(isset($this->gen_contents['subregion_search']))
				...
				$subregion_id	= 0;
                        ...
                        $chp_list = $this->config->item('chapter_list');
                        //$data["course_list"] = $this->admin_career_event_model->dbSelectAllCourses();
                        $data["course_list"] = $this->admin_career_event_model->dbGetMonthlyCourses(date('Y/m/d',$first_day_of_month),date('Y/m/d',$last_day_of_month), $region, $sub_region);
			...
			$data['events']                 = $this->admin_career_event_model->dbGetMonthlyEventDetailsUser(date('Y/m/d',$first_day_of_month),date('Y/m/d',$last_day_of_month),$region_id,$subregion_id);
                        $data['events_clr']		= $this->admin_career_event_model->dbGetMonthlyEventDetailsUserclr(date('Y/m/d',$first_day_of_month),date('Y/m/d',$last_day_of_month),$region_id,$subregion_id);
                        $data['events_crs']		= $this->admin_career_event_model->dbSelectAllCoursesarr();
		...
		function class_details() {
			...
			$this->gen_contents['arr_class'] 		= $this->admin_career_event_model->dbGetSingleEventDetails($_POST['masterid']);
			...
			$this->gen_contents['adhi_course'] = $this->admin_career_event_model->dbSelectSelcCrashCourse();
			$this->load->view('user/career_event/display_schedule_details', $this->gen_contents);
		}
