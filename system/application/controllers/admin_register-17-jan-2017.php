...
class Admin_Register extends Controller
{
		...
		var $current_controller = 'admin_register';
	...
	function Admin_Register(){
		parent::Controller();
			...
			$this->gen_contents['css'] = array('style.css','dhtmlgoodies_calendar.css','admin_register.css','admin_style_main.css', 'admin_style.css' );
	...
	function _init_registration_rules()
	{
		...
		$this->form_validation->set_rules('driving_license', 'Driving License', 'required|max_length[10]');
	}
	...
	function _init_user_regdetails()
	{
		...
		function _set_step1_registration_details($step1_arr){
			$this->gen_contents['step1_data'] = array(
				...
				"driving_license" 	=> 	$step1_arr['driving_license'],
...
//                        print_r($_POST);
            ...
            if ($this->input->post('step3') == 3 and $this->session->userdata('step2') == 2)
            {
                ...
                if (!empty($_POST))
                {
                    ...
                    if ($this->form_validation->run() == TRUE)
                    {
                        $check =$this->user_model->checkuser($this->session->userdata('emailid'));                                                
                        if($check > 0){
                            $this->gen_contents["msg"] = "Email id already exists";
                            $this->_int_user_register_course();
                            return true;
                        ...
                        //Logout for deleted subadmin
                         ...
                         if(count($subadmin_data) == 0) 
                         {
                             ...
                             redirect(c('admin_login_url').'/sub');
                             exit;
...
/**
        ...
        function list_user_details (){
                $this->authentication->check_permission_redirect('sub_permission_1');
                ...
                $config['base_url'] 				= 	base_url().'index.php/admin_register/list_user_details/';
                $config['per_page'] 				= 	'10';
        ...
        function _profile_progress($user_id, $email_id){
            ...
            if($user_stat['completed_all_exams'] && $this->course_model->isCrashCourseUser($email_id)) {
                ...
                $user_stat['state_exam_applied']        = array('class' => '');// only 'active' class will be clickable
                ...
                if($profile_progress && $this->_item_key_exists($profile_progress, 'state_exam_applied')){
                    ...
                    $user_stat['obtained_license']      = array('class' => '');
