...
// ------------------------------------------------------------------------
	class Admin_user extends Controller
	{
		...
		var	$b_state		=	'';
		var	$b_country		=	'';
		var	$current_controller     =	'admin_user';
		/**
		 * Admin constructor
		 *
		 */
		...
		function _init_user_details (){
			...
			if($this->input->post('driving_license')){
                            $this->driving_license = $this->common_model->safe_html($this->input->post('driving_license'));
                        }
		...
		function list_user_details ()
		{
			...
			if(!empty($_POST)) {
				...
				$this->gen_contents["course_type"]      = $this->session->flashdata('course_type');
			...
			$this->session->set_flashdata('license_type',$this->gen_contents["license_type"]);
                        ...
                        $this->session->set_flashdata('search_city',$this->gen_contents["search_city"]);
                        $this->session->set_flashdata('search_zipcode',$this->gen_contents["search_zipcode"]);
			/*****sree 080710**/
			if($this->gen_contents["course_completed"]){
				$is_completed = (-1 == $this->gen_contents["course_completed"]) ? false: true;
				$this->gen_contents["userdetails"]	= $this->admin_user_model->select_userdetails_completed($config['per_page'],$this->uri->segment(3),$this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"], $this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"], $this->gen_contents["course_type"], $is_completed);

				$config['total_rows']   		= 	$this->admin_user_model->qry_count_userdetails_completed($this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"],$this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"], $this->gen_contents["course_type"],$is_completed);

			}else{

				$this->gen_contents["userdetails"]	=	$this->admin_user_model->select_userdetails($config['per_page'],$this->uri->segment(3),$this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"], $this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"],$this->gen_contents["course_type"]);
				$config['total_rows']   		= 	$this->admin_user_model->qry_count_userdetails($this->gen_contents["search_firstname"],$this->gen_contents["search_lastname"],$this->gen_contents["search_email"],$this->gen_contents["license_type"],$this->gen_contents["search_phone"],$this->gen_contents["search_zipcode"], $this->gen_contents["search_city"], $this->gen_contents["course_type"]);
			}
		...
		function _edit_user_details ($userid){
			...
			$userarray	=	array(
                        ...
                        if(isset($this->driving_license) && '' != $this->driving_license){                            
                            $userarray['driving_license'] = generate_hash($this->driving_license);
                        }
			...
			if($this->form_validation->run() == TRUE) {
				...
				if(0 == count($emailexist)){
					...
					if(count($emailexist) > 0){
						$this->session->set_flashdata ('error', 'Email id already exists. Please choose another one');
		...
		function freeze_user() {
			...
			if($freeze_user >0)
			{
				$this->session->set_flashdata ('success', 'User has been disabled successfully');
		...
		function edit_effective_date () {
			...
			$this->gen_contents['ship_status']      = 	$this->admin_user_model->get_ship_status($this->gen_contents['courseid'], $this->gen_contents['userid']);
                        if($this->_validate_effective_date($this->gen_contents)){
                            ...
                            if($update > 0){
                                $this->load->model('user_exam_model');
                                $examlist = $this->user_exam_model->getquestionid ($this->gen_contents['courseid'],$this->gen_contents['edition']);
                                if (count($examlist)==0){
                                    $this->session->set_flashdata ('error', '<br/><br/> <u> Note </u> : Admin has not uploaded questions in this edition');
                                }
...
exit();*/
        ...
        function view_user_exam_details(){
			...
			$user_exam_details	= $this->user_exam_model->getUserAttendedDetails($this->userid, $courseid);
                        $previous_attempts	= $this->user_exam_model->getPreviousAttempts($this->userid, $courseid);
			...
			if($user_exam_details){
                ...
                $enable_random_selection = false;
				$question_all	= $this->user_exam_model->getquestionid($courseid,$edition, $ordered_question_ids, $enable_random_selection);
				...
				$this->gen_contents['page_title']	= 'User Exam Details';
                                $this->gen_contents["previous_attempts"]		=  $previous_attempts;
        ...
        function edit_otp_user()
	{
            ...
            if($user){
                if($_SERVER['REQUEST_METHOD'] == 'POST') {  
                    ...
                    if($this->form_validation->run() == TRUE) {
                        ...
                        if($check_in_trial){
                            if(0 == $check_in_trial->status){
                                $this->gen_contents['msg']   = 'Email id already exists with "activation pending" status.';
                            }else if(1 == $check_in_trial->status){
                                $this->gen_contents['msg']   = 'Email id is already exists.';
                            }else if(2 == $check_in_trial->status){
        ...
        function adhi_sale_list($date_onwards ='2017-01-01'){
            $this->load->model('admin_user_model');
            //$date_onwards = '2017/01/01';
            //die($date_onwards);
            $date_onwards = date('Y/m/d', strtotime($date_onwards));
            $this->admin_user_model->getAdhiSaleList($date_onwards);
        }
        
        function adhi_new_sale_list(){
            $this->load->model('admin_user_model');
            $this->admin_user_model->getAdhiNewSaleList();
        }
       ...
       function list_export_to_excel() {
           ...
           if(!empty($_POST)) {
                ...
                $search_params["course_completed"]  = $this->common_model->safe_html($this->input->post('c_completed'));
                ...
                $search_params["course_type"]       = $this->common_model->safe_html($this->input->post('course_type'));
            ...
            /*****sree 080710**/
            if(1 == $search_params["course_completed"]){
                $users  = $this->admin_user_model->select_userdetails_completed(0,0,$search_params["first_name"],$search_params["last_name"],
                        $search_params["email"],$search_params["license_type"],$search_params["phone"],$search_params["zipcode"],$search_params["city"],
                         $search_params["course_type"],true,false);
            }else if(-1 ==$search_params["course_completed"]){
                $users  = $this->admin_user_model->select_userdetails_completed(0,0,$search_params["first_name"],$search_params["last_name"],
                        $search_params["email"],$search_params["license_type"],$search_params["phone"],$search_params["zipcode"],$search_params["city"],
                         $search_params["course_type"],false,false);
            }else{
                $users  = $this->admin_user_model->select_userdetails(0,0,$search_params["first_name"],$search_params["last_name"],
                        $search_params["email"],$search_params["license_type"],$search_params["phone"],$search_params["zipcode"],$search_params["city"],
                         $search_params["course_type"],false);
            }
           ...
           if(!empty($users)){
               ...
               $this->excel->getActiveSheet()->setCellValue('E4', 'City');
               ...
               $this->excel->getActiveSheet()->mergeCells('A1:E3');
               ...
               foreach ($users as $data){
                    ...
                    $this->excel->getActiveSheet()->setCellValue('E'.$row,$data->city);
       ...
       function change_trial_status() {
                $userid 	= 	$this->uri->segment(4);
                $status         = 	$this->uri->segment(3);
                
                if($userid > 0 && '' != $status){
                    $user   = $this->admin_user_model->isTrialUserExists($userid);
                    if($user){
                        if((1 == $user->status && 'freeze' == $status) || (4 == $user->status && 'unfreeze' == $status)){
                            $change_status  = ('freeze' == $status) ? 4 : 1;
                            $action_text    = ('freeze' == $status) ? "Enabled" : "disabled";
                            $change_status_reason = '<i>'.$action_text.' @'.date('m/d/Y h:i a', strtotime(convert_UTC_to_PST_datetime(date('Y-m-d H:i:s')))).'</i><br/>'.$this->input->post('txtReason').'<br/><br/>'.$user->change_status_reason;
                            $update_data = array(
                                    'status'                        => $change_status,
                                    'change_status_reason'          => $change_status_reason, 
                                    'updated_at'                    => convert_UTC_to_PST_datetime(date('Y-m-d H:i:s'))
                                    );
                            $result         = $this->admin_user_model->changeTrialUserStatus($userid, $update_data);
                            
                            $page           = $this->uri->segment(5) ? $this->uri->segment(5) : 0;

                            if($result){
                                $this->session->set_flashdata ('success', 'User has been '.$action_text.' successfully');
                                redirect('admin_user/list_trial_users/'.$page);
                            }else{
                                $this->session->set_flashdata ('error', 'Request Failed');
                                redirect('admin_user/list_trial_users/'.$page);
                            }
        ...
        function reenroll(){
            ...
            if("" != $user_id && "" != $course_id){
                $this->load->model('admin_model');
                $result = $this->admin_model->getUserDetails($user_id);
                $course = $this->admin_model->getCourse($course_id);
                $user_course = $this->admin_model->getUserCourse($user_id,$course_id);
                $course_details = array('reg_courseid' => $user_course->id,'renew_date' => convert_UTC_to_PST_date(date("Y-m-d")),
                    'b_address' => $result->b_address,'b_city' => $result->b_city,'b_state' => $result->b_state,'b_country' => $result->b_country,'b_zipcode' => $result->b_zipcode);
                
                //$this->session->set_flashdata('success',"Renewed successfully");
                $this->admin_model->adminrenewcourse($course_details);
                
                $this->admin_model->send_renewal_mailto_user($result->emailid, $result->firstname, $course->course_name, $result->course_user_type, $course->amount);
                exit;
            }
         ...
         *
         */
        function view_user_other_exam_details(){
        	$this->load->model('user_exam_model');
        	$this->userid 		= $this->uri->segment(3);
        	$courseid 			= $this->uri->segment(4);	
                $track_id 			= $this->uri->segment(5);
			if('' == $this->userid || '' == $courseid){
				redirect('admin_user/list_user_details');
			}
			$user_exam_details	= $this->user_exam_model->getUserPreviousAttendedDetails($this->userid, $courseid, $track_id);
			$question_details	= array();
			$option_details		= array();
			$right_answer_count	= 0;
			$wrong_answer_count	= 0;
			...
			if($user_exam_details){
				...
				$this->gen_contents["other_exam_details"]		= $this->user_exam_model->getUserOtherAttendedDetails($this->userid, $courseid);
                                $this->gen_contents["previous_attempts"]                = array();
				$this->gen_contents['page_title']	= 'User Exam Details';
				$this->_template('view_exam_details', $this->gen_contents);
			}else{
				redirect('admin_user/user_course_details/'.$this->userid);
        ...
        function students_list($date_onwards ='2018-07-24'){
            $this->load->model('admin_user_model');
            $date_onwards = date('Y/m/d', strtotime($date_onwards));
            $this->admin_user_model->getStudentList($date_onwards);
			exit;
        }
	
		/**
         * Update student reg date from enrolled date
         *
         */
        function update_students_reg_date(){
            $this->load->model('admin_user_model');
            $this->admin_user_model->updateStudentRegDate($date_onwards);
			exit;
        }

        function expired_student_all_course_incomplete(){
            $this->load->model('admin_user_model');

            $this->admin_user_model->getExpiredStudentListWhoNotCompletedAnyCourse();
            exit;
        }
        
        function adhi_december_sale_list(){
            $this->load->model('admin_user_model');
            $this->admin_user_model->getAdhiDecemberSaleList();
        }
        
        function editionThreeUser(){
            ini_set("display_errors",1);
                error_reporting(E_ALL);
            $this->load->model('admin_user_model');
            $this->admin_user_model->getEditionThreeUsers();
        }
        
        function certificate_download_history(){
            $this->load->model('admin_user_model');
            $this->admin_user_model->getCertificateDownloads();
        }
