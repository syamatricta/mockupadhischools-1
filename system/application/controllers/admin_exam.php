				...
				$edition_id			= 	$this->input->post('edition');
			 	...
			 	if (($_FILES['userfile']['error']) ==  0) {
					...
					if($this->do_upload()){
						...
						$replace_old_questions	=	$this->input->post('replace_old_questions');
						...
						if($msg=read_excel_validate($xls_path,'','',$course_id)){//validation for xls file
							...
							if($this->admin_exam_model->saveExamDetails ($xls_path,$course_id,$edition_id)){
                                ...
                                $result = read_excel($xls_path,$course_id,$edition_id);
								if($result['inserted'] > 0){
								    $message = 'Successfully inserted '.$result['inserted'].' questions. out of '.$result['total'];
								    if($result['duplicates'] > 0){
                                        $message    .= ' '.$result['duplicates'].' question(s) duplicated.';
                                    }

                                    $this->session->set_flashdata('success', $message);
                                }else{
                                    $message = 'Failed to insert questions out of '.$result['total'].'. Duplicate entry';
