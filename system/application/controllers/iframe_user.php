		...
		function _init_user_regdetails_step2(){
			...
			if($this->Common_model->safe_html($this->input->post('txthowhear')) == 'Search engine'){
				...
				$reason = $this->Common_model->safe_html($this->input->post('txtREO'));
                        ...
                        if(end($this->uri->segment_array()) == 'alam'){
                            $reason .= " - ALAM";
                        }else if(end($this->uri->segment_array()) == 'associates'){
                            $reason .= " - ASSOCIATES";
                        }else{
                            $reason .= " - KWADHI";
                        }
...
//					"unit_number"	=>	$this->Common_model->safe_html($this->input->post('unitnumber')),
       ...
       function _user_forum_data($transactionid){
                        ...
                        if($this->form_validation->run() == TRUE) {
                            ...
                            if($check <= 0 ){
                                ...
                                //  if($checkins > 0){
                                        ...
                                        if(end($this->uri->segment_array()) == 'associates'){
                                            $created_by = 3;
                                        }else if (end($this->uri->segment_array()) == 'alam'){
                                            $created_by = 2;
                                        }else{
                                            $created_by = 1;
                                        }
		...
		function register() {
            ...
            if(end($this->uri->segment_array()) == 'alam'){
                $this->gen_contents['site'] = "alam";
            }else if(end($this->uri->segment_array()) == 'associates'){
                $this->gen_contents['site'] = "associates";
            }else{
                $this->gen_contents['site'] = "";
            }
		...
		function register_step2(){
			...
			if($this->input->post('step2') == 2 && $this->session->userdata('step1') == 1 ){
				...
				if(!empty($_POST)) {
  				...
  				redirect("iframe_user/register/".end($this->uri->segment_array()));
			}
		}
		...
		function courseadd(){
				...
				$this->load->helper(array("form", "fedex_helper"));
						...
						if("SUCCESS" == strtoupper($data['payment']["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($data['payment']["ACK"])) {
							...
							**/							
						    ...
						    $this->_user_forum_data($data['payment']["TRANSACTIONID"]);
                                ...
                                $courseDetails['packageType'] = $this->order_contents['packaging_type'];
                                ...
                                $aryOrder   = get_fedex_order_array(count($this->input->post('course')));
...
/*			$this->load->view("client_common_header",$this->gen_contents);						
			...
			$this->load->view("client_common_footer");*/
                ...
                function contact() {
                        ...
                        if(end($this->uri->segment_array()) == 'alam'){
                            ...
                            $this->gen_contents['site'] = 'associates';
                            $from = "Associates Realty";
                        }else{
                            $this->gen_contents['site'] = 'kwadhi';
                            $from = "Keller Williams Realty";
                        }
                        $this->gen_contents['error'] = '';
                        $this->gen_contents['success'] = '';
                        
			if(!empty($_POST)){
                            
                            if(array_key_exists('email',$_POST)){
                                $this->gen_contents['error'] = 'Please try again !';

                                $this->load->model('Common_model');
                                $toemail    = $_POST['email'];
                                $subject1   = 'Enquiry from '.$from;
                                $contents1  = 'Dear Admin, <br/><br/>'
                                        . ucfirst($_POST['first_name']).' '.ucfirst($_POST['last_name']).' with email id '.$_POST['email'].' has requested for a callback.
                                          <br/> His/her phone number is '.$_POST['phone_number'].'.<br/> <br/> Thanks, <br/> Team '.$from;
                                if($this->Common_model->send_mail($this->config->item('guest_pass_enquiry'),$from,$subject1,$contents1,'')){
                                    $subject2   = 'Enquiry Response - '.$from;
                                    $contents2  = 'Hi '.ucfirst($_POST['first_name']).',<br/> <br/>'
                                            . 'Thank you for contacting us. Our customer service representative will get back to you. <br/><br/>'
                                            . 'Team '.$from.'<br/><br/> <i> This is an auto generated email. Please do not reply back. </i>';
                                    $this->Common_model->send_mail($toemail,$from,$subject2,$contents2,'');
                                    $this->gen_contents['error']   = '';
                                    $this->gen_contents['success'] = 'Enquiry sent !';
                                }
                            }
                        }	
                    
                    $this->load->view('iframe_user/contact/contact',$this->gen_contents);
    ...
    function _init_package($courseDetails){
        ...
        $packageType        = $courseDetails['packageType'];
        ...
        $packageDetails[0]     = array_merge($packageDetails[0], get_fedex_packaging_dimension($packageType));
    ...
    function _init_recipient(){
        ...
        $this->aryRecipient = array(
            ...
            'Address' => array(
                'StreetLines'           => s('address').', '.s('unit_number'),
                'City'                  => s('city'),
