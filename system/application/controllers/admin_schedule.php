...
// ------------------------------------------------------------------------ 
	class Admin_schedule extends Controller
	{
		...
		function add_event(){
			...
			$schedule_chapters  = $this->admin_schedule_model->getAllScheduleChapters();
                        if($schedule_chapters){
                            $schedule_chapters_json = array();
                            foreach ($schedule_chapters as $chapter){
                                if(array_key_exists($chapter->course_id, $schedule_chapters_json)){
                                    array_push($schedule_chapters_json[$chapter->course_id], array($chapter->id => $chapter->title));
                                }else{
                                    $schedule_chapters_json[$chapter->course_id]    = array($chapter->id => $chapter->title);
                                }
                                
                            }
                        }
                        $this->gen_contents['schedule_chapters']= json_encode($schedule_chapters_json);
		...
		function _dbEventProcessor(){
			...
			if(isset($_POST['txtWhat2Do'])){
				...
				if($_POST['txtWhat2Do'] == 'add' || $_POST['txtWhat2Do'] == 'edit'){ //add events to calendar
					...
					$chapters_with_date  = array();
                                        if(isset($_POST['txtRepeatWeeklyChapters']) && 'yes' == $_POST['txtRepeatWeeklyChapters'] && 5 == $_POST['sltCourses']){
                                            $schedule_chapters  = $this->admin_schedule_model->getAllScheduleChapters($_POST['sltCourses']);
                                            
                                            $add_string     = "+0 day";
                                            $sCurrentDate   = date("Y/m/d", strtotime($arr_events['date']));                    
                                            foreach($schedule_chapters as $chapter){
                                                $sCurrentDate = date("Y/m/d", strtotime($add_string, strtotime($sCurrentDate)));
                                                $chapters_with_date[$chapter->id] = array(
                                                    'date'  => $sCurrentDate,
                                                    'schedule_chapter_id' => $chapter->id,
                                                    'desc'  => $chapter->title
                                                );
                                                $add_string = "+7 day";
                                            }
					...
					if($_POST['txtWhat2Do'] == 'add'){
						$ret_arr = array();
						$ret_arr = $this->admin_schedule_model->dbInsertEventMaster($arr_events,$cc_arr_sub_events, $chapters_with_date);
						if(empty($ret_arr)){							
					...
					if($_POST['txtWhat2Do'] == 'edit'){
						...
						if($course_id==15){
							$this->admin_schedule_model->dbDeleteEventDetailsCCO($_POST['hdnMasterid']);
                                                ...
                                                if($this->admin_schedule_model->isWeeklyScheduledChapter($_POST['hdnMasterid'])){
                                                    $instructor_id = @$_POST['instructor'];
                                                    $this->admin_schedule_model->updateWeeklyScheduledDetails($_POST['hdnMasterid'], $arr_events, $instructor_id);
                                                }else{
							...
							if(empty($ret_arr)){									
								...
								$this->gen_contents['success_message'] 	= 'Successfully updated the event(s)';
                                        ...
                                        $masterid   = $_POST['hdnMasterid'];
                                        if($group_master_id = $this->admin_schedule_model->isWeeklyScheduledChapter($masterid)){
                                            if($this->admin_schedule_model->deleteSuccessiveChapters($masterid, $group_master_id)){
                                ...
                                else if($_POST['txtWhat2Do'] == 'cancel'){
					if(!$this->admin_schedule_model->cancelScheduleChapter($_POST['hdnMasterid'])){
						$this->gen_contents['error_message'] 	= 'Event cancel failed';
					}else{
						$this->gen_contents['success_message'] 	= 'Successfully postponed all coming event(s) to next weeks';
					}						
				}
		...
		function class_display(){
			...
			$this->display_calendar();
                ...
                function _getRepeatChapterDays($sStartDate, $no_of_weeks){
                    $sStartDate = date("Y/m/d", strtotime($sStartDate));
