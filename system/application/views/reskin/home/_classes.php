...
<section id="section-class" class="section-class">
        ...
        <div class="row"> 
	      	<div class="col-sm-12">
		      	<div class="wrapper-with-margin"> 
		      		 <p class="text-right loc-cnt wow fadeInUp"><a class="loc" href="<?php echo base_url()?>find-real-estate-classes">View all locations &nbsp;<i class="fa fa-arrow-right"></i> </a></p>
		      	<div id="classess_view" class="owl-carousel">  
		      		<?php $i = 0;
					//$image_path = $this->config->item('image_upload_url');
					$image_path = $this->config->item('site_ssl_baseurl').'image_uploads/thumbs/';
					foreach($arr_class as $val):
						$i++;
						//$full_image = $this->config->item('image_upload_path').$val->image;
                        $full_image = $this->config->item('site_ssl_baseurl').'image_uploads/thumbs/'.$val->image;
						if($val->image && file_exists(c('site_basepath').'image_uploads/thumbs/'.$val->image)){
	  					...
	  					<div class="item wow fadeInUp" data-wow-delay="<?php echo $i*.5;?>s">
	  						<div class="course_section">
	  							<div class="class-img">
                                    ...
                                    src="<?php /*echo base_url()?>timthumb.php?src=<?php echo $full_image ; */?>&w=320&q=100&h=246"
                                    -->
                                    <a href="<?php echo base_url(); ?>home/open_location/<?php echo $val->subregion_id; ?>" class="today-class-img" style="background-image: url(<?php echo $full_image;?>)">
                                    </a>
                                </div>
		  						
	                  			<h4 class="class-name"><?php echo $val->course?></h4>                  			 
                                <div class="class-details" data-location="<?php $loc = $val->region.' , '.$val->subregion;echo htmlentities($loc);?>"
                                                        data-short-location="<?php echo strlen($loc) > 35 ? htmlentities(substr($loc, 0, 35))."..."  : htmlentities($loc);?>"
                                                 >
                                    <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <div class="class-title cls_c_chapter" data-description="<?php echo htmlentities($val->subregion_description)?>"
                                                data-chapter-desc="<?php echo htmlentities($val->descp);?>"
                                                ><?php echo strlen(strip_tags($val->descp)) > 45 ? substr(strip_tags($val->descp), 0, 45)."..." : substr(strip_tags($val->descp), 0, 45)?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <?php /*<div class="modal fade" tabindex="-1" role="dialog" id="course_details">
