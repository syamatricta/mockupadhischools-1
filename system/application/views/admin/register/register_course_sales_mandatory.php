<?php if($license == 'S'){?>
    ...
    <div  class="filedforrate"  >
        ...
        <input type="checkbox" class="scheck" name="course[]"   id="course<?php echo $courses_m['course_id']; ?>" value="<?php echo $courses_m['course_id']; ?>" onClick="javascript:showterms(this.value), checkrate();"  />
        <?php //if($courses_m['child_cnt'] ==0){ ?>
					<label for="course<?php echo $courses_m['course_id']; ?>" ><?php echo $courses_m['course_name'] ." -  $".$courses_m['amount']; ?></label>
		  		<? /*}
		  		else {
	    ...
	    <div class="filedforrate paddingbottom" id="showdiv<?php echo $courses_m['course_id']; ?>" style="display:none;" >
	    	<div class="floatleft">
			    ...
			    <div  class="filedforterm agreement" >
			    	<div class="agreementagreecheck"><input type="checkbox" name="agree<?php echo $courses_m['course_id']; ?>" id="agree<?php echo $courses_m['course_id']; ?>" value="<?php echo $courses_m['course_id']; ?>" onclick="javascript:showcheck(this.value),checkrate();"></div>
			       	<div class="admin_agreementagreetext"> <label for="agree<?php echo $courses_m['course_id']; ?>">I Agree</label></div>
			        <div class="floatleft"><input type="checkbox" name="disagree<?php echo $courses_m['course_id']; ?>" id="disagree<?php echo $courses_m['course_id']; ?>" value="<?php echo $courses_m['course_id']; ?>" onclick="javascript:showuncheck(this.value),checkrate();"  /></div>
			        <div class="admin_agreementdonttext"> <label for="disagree<?php echo $courses_m['course_id']; ?>">I Don't Agree </label></div>
			     </div>
		    </div>
		</div>
	    <div class="clearboth">&nbsp;</div>
