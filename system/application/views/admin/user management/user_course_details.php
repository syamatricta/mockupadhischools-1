					...
					<?php if($this->authentication->check_permission_redirect('super_admin', FALSE)){ ?>
                                            ...
                                            <?php $curdate = convert_UTC_to_PST_date_slash(date('m/d/Y H:i:s')); ?>				
                                                    ...
                                                    <?php if($data->exam_attended_exist){?>
                                                            ...
                                                            if($data->need_to_reenroll == 1 && $data->attempt_count_reenroll == 2 && 1 == $reenroll_status) { ?>
                                                                    <a class="action_icon" style="clear:left;" href="javascript:void(0);" onclick="confrm_reenroll('<?php echo $data->courseid;?>','<?php echo $userid;?>');">Re-enroll</a>
                                                            <?php }
                                                            ?>
...
function confrm_reenroll(course_id, user_id)
{
    if(confirm("Are you sure to reenroll this course?"))
    {
        var url  =   base_url + '<?php echo $this->current_controller;?>' +"/reenroll";
        var params      =   'course_id='+course_id+'&user_id='+user_id;
        new Ajax.Request(url,{
               method      : "post",
               onSuccess   : reenroll_success,
                             parameters  : params,
               onFailure   : disp_delete_course_error
             }
        );
...
function reenroll_success(resp_obj)
{
    if(resp_obj.responseText == "error") 
    {
        $("errordisplay").innerHTML = "Error! Please tr after sometime";
    }
    else
    {
