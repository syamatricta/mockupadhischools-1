...
class Admin_user_model extends Model{
	...
	function select_single_user_order_details($userid){
		...
		$this->db->select ("id,userid,transactionid,total_amount,orderdate,delivered_date,trackingno,label_path,last_trackdate,current_location,status,ship_method,ship_rate,course_price,b_state,b_city,b_address,b_zipcode");
...
//             print_r(count($tot));
	...
	function changeTrialUserStatus($id, $data){
       ...
       function getCrossRefUsers(){
            ...
            /* Trail users with not moved to as user, fetching pending, active trail and expired */            
            ...
            /* Attempted to register not registered */            
        ...
        function getAdhiSaleList(){
            $this->db->select('reg_first_name,reg_last_name, reg_email,reg_date');
            $this->db->where(array('status' => 1,"DATE_FORMAT(reg_date, '%Y/%m/%d') >=" => '2016-01-01'));
            ...
            $this->db->from('adhi_trial_users');
            $this->db->where("DATE_FORMAT(created_at, '%Y/%m/%d') >=" , '2016-01-01');
            $this->db->order_by('created_at','ASC');
            $guest_list = $this->db->get();
            
            
            $this->db->select('new_user_name, new_user_email_id,created_date');
            $this->db->where(array('cron_status <=' => 1,"DATE_FORMAT(created_date, '%Y/%m/%d') >=" => '2016-01-01'));  
            ...
            if($reg_in_process_list ->num_rows() > 0){
                ...
                if(!empty($reg_in_process_list)){
                    ...
                    foreach($reg_in_process_list as $key => $unsub){
                        $reg_in_process_list[$key]['reg_email'] = strtolower($reg_in_process_list[$key]['reg_email']);
                    }
                    
                    foreach($reg_in_process_list as $unsub){
                        
                        if(!in_array($unsub['reg_email'],$email_arr)){
                            $email_arr[$s] = $unsub['reg_email'];
            ...
            if($guest_list ->num_rows() > 0){
                ...
                if(!empty($guest_list)){
                    ...
                    foreach($guest_list as $key => $unsub){
                        $guest_list[$key]['email'] = strtolower($guest_list[$key]['email']);
                    }
            ...
            if($unsub_list ->num_rows() > 0){
                ...
                foreach($unsub_list as $key => $unsub){
                    $unsub_list[$key]['new_user_email_id'] = strtolower($unsub_list[$key]['new_user_email_id']);
                }
            ...
            ?>
                ...
                <?php if(!empty($date_arr)){ 
                 ...
                 <tr> 
                        ...
                        <td colspan="2"> <?php echo $email_arr[$key]; ?> </td>
                        <td colspan="2"> <?php echo $date_arr[$key]; ?> </td>
                 </tr>     
                <?php $i++;
                } 
                }else { ?>
        ...
        function getAdhiNewSaleList(){
            ...
            if(!empty($user_process_list)){
                ...
                foreach($user_process_list as $key => $us){
                    $user_process_list[$key]['emailid'] = strtolower($user_process_list[$key]['emailid']);
                }
                ...
                foreach($user_process_list as $key => $us){
                    $date_diff = find_date_diff(date('Y-m-d', strtotime($us['enrolled_date']."+2 years")),convert_UTC_to_PST_date(date('Y-m-d')));
