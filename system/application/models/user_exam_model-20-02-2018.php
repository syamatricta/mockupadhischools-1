...
class User_exam_model extends Model{
	...
	function get_course ($id) {
		...
		if(isset ($id) && '' != $id){
			$sql="SELECT c.course_name,c.exam_status,c.course_code,c.id from  adhi_courses as c join adhi_user_course as l on 
	...
	function getquestionid($id,$edition){
		
		if(isset ($id) && '' != $id){
			$sql="SELECT c.id ,l.id as ansid,c.questions as question from  adhi_exam_questions as c join adhi_exam_answers as l on 
				c.id = l.question_id  where c.course_id ='".$id."' AND c.edition ='".$edition."' AND l.answer_option='Y' ORDER BY RAND() limit 0,100";
			//	echo $sql;die();
				$query= $this->db->query($sql);
	...
	*/
	...
	function get_userinfo($id,$courseid){
		...
		c.parent_course_name,c.course_code,c.course_name
	...
	function create_pdf($arr,$unam=''){
				...
				$this->fpdf->Cell(100);
				$this->fpdf->cell(100,10,"5000 Birch Street",0,0,'C');
				$this->fpdf->Ln(5);
				$this->fpdf->Cell(100);
				$this->fpdf->SetFont('Arial','BI','11');
				$this->fpdf->cell(100,10,"West Tower",0,0,'C');
				$this->fpdf->Ln(5);
				$this->fpdf->Cell(100);
				$this->fpdf->SetFont('Arial','B','11');
				$this->fpdf->cell(100,10,"Suite 3000",0,0,'C');
				
				$this->fpdf->Ln(5);
				$this->fpdf->Cell(100);				
				$this->fpdf->cell(100,10,"Newport Beach, CA 92660",0,0,'C');
				...
				$this->fpdf->Ln(5);
				$this->fpdf->Image($this->config->item('images').'/adhi_logo.jpg',10,12,30,0,'','http://www.adhischools.com');
				$this->fpdf->Ln(5);
				$this->fpdf->SetFont('Arial','B','11');
				$this->fpdf->SetX(20);
				$this->fpdf->SetY(45); 
				...
				$this->fpdf->SetX(20);
				$this->fpdf->SetY(55); 
				$heading="Statutory Course Completion Certificate for BRE Approved Courses";
				...
				$this->fpdf->MultiCell(18, 4, 'BRE Approval', 1,1);	
				$x = $this->fpdf->GetX();
				$y = $this->fpdf->GetY();
				...
				$this->fpdf->Image($this->config->item('images').'adhi_sign.jpg',40,$y-5,20,20,'');
				$this->fpdf->Cell(45);
