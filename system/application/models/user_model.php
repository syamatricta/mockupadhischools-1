  ...
  /*require_once $this->config->item ('site_basepath').'system/application/libraries/vbintegration.php';
	...
	function send_mailto_user($mail,$order,$trackinfo,$from_admin='',$usertype,$ship='yes'){
		...
		$contents_live		= '';
            ...
            if(isset($order['is_promocode_applied']) && $order['is_promocode_applied'] == 1){   
                $promocode_details   = json_decode($order['promocode_details'], TRUE);
                $discount       = (1 == $promocode_details['redeem_type'] ) ? '$'.$promocode_details['redeem_value'] : $promocode_details['redeem_value'].'%';
                $contents	.='<tr>
					<td align="left"  width="150" >Promocode Applied </td>
					<td align="left"  width="150" > '.$discount.' discount availed </td>
				</tr>';
            }                
	...
	function get_the_course_type($course_user_type = FALSE){
        ...
        function get_all_courses($user_id = FALSE){
                if($user_id){
                    $query= $this->db->query("select * from  adhi_user_course where userid = $user_id");
                    return $query->result_array();
                }
                return true;
	}
	//end
        ...
        function updateTheEffectiveDate($arr,$id){
            $this->db->where('id', $id);
            $this->db->update('adhi_user_course', $arr);
	}
        
        function getPromocodeDetails($code){
            $this->db->where('code', $code);
            $query  = $this->db->get('adhi_promo_codes');
            return $query->row();
	}
        
        function getAllPromocodes(){
            $this->db->where("start_at < '".date('Y-m-d H:i:s')."' AND end_at > '".date('Y-m-d H:i:s')."'", NULL, FALSE);
            $query  = $this->db->get('adhi_promo_codes');
            return $query->result();
	}

	function courseContactExists($field, $value){
        $this->db->where($field, $value);
        $query  = $this->db->get('adhi_course_contact');
        if($query->num_rows()){
        	return true;
		}
        return false;
	...
	function saveCourseContact($data){
        $this->db->set($data);
        return $this->db->insert('adhi_course_contact');
	}
        
        function third_party_user(){
            $data = array(
                'ip_address' => $this->input->ip_address(),
                'created_date' => convert_UTC_to_PST_datetime(date('Y-m-d H:i:s')),
                'status' => 1
            );
            $this->db->insert('adhi_third_party_users', $data);
            $adhi_user_id   = $this->db->insert_id();
            return $adhi_user_id;
        }
        
        function clear_reenrolls($user_id = 0,$course_id = 0, $renroll = 0){
            if($user_id && $course_id){
                $arr = array("attempt_count_reenroll" => 0, "need_to_reenroll" => 0);
		...
		$this->db->update('adhi_user_course', $arr);
                ...
                if($renroll){
                    $data = array(
                        'user_id' => $user_id,
                        'course_id' => $course_id,
                        're_enrolled_date' => convert_UTC_to_PST_datetime(date('Y-m-d H:i:s')),
                        'status' => 1
                    );
                    $this->db->insert('adhi_renroll_list', $data);
                }
            }
        }
        
        function effective_update(){
            $arr = array("attempt_count_reenroll" => 0, "need_to_reenroll" => 0, "effective_date" => "2018-12-15",
                "last_attemptdate" => "0000-00-00", "status" => "","final_score" => 0);
            $this->db->where('userid', '10138');
            $this->db->update('adhi_user_course', $arr);
            
            $this -> db -> where('user_id', '10138');
            $this -> db -> delete('adhi_exam_tracking');
        }
        
        function effective_broker_update(){
            $arr = array("attempt_count_reenroll" => 0, "need_to_reenroll" => 0, "effective_date" => "2018-12-15",
                "last_attemptdate" => "0000-00-00", "status" => "","final_score" => 0);
            $this->db->where('userid', '7055');
            $this->db->update('adhi_user_course', $arr);
            
            $this -> db -> where('user_id', '7055');
            $this -> db -> delete('adhi_exam_tracking');
        }
        
        function emailidExists($emailid = "", $userid =""){
            if("" != $emailid){
        ...
        function updateUserDetails($userid, $userdetail){
            $this->db->where('id', $userid);
            $this->db->update('adhi_user', $userdetail);
        }
        
                function send_renewal_mailto_admin($mail,$usertype){
			$this->load->model('Common_model');
		...
		$subject='Renewal Details';
		$contents		= '';
		$contents		.= 'Dear Admin,<br><br>';

		$contents		.= $mail['name']." have successfully renewed ".$mail['coursename']." <br><br>";
		$contents		.= '<b>Payment Details</b><br>';
			$contents		.=
			'<table cellpadding="4" cellspacing="0" border="1" width="300"  style="border:#CCCCCC thin;">
        ...
        function temp_paymentlog($arr){
            $this->db->insert('temp_adhi_payment_log', $arr);
            return $this->db->insert_id();
	}
        
        function temp_userregistration($arr, $register_user=0){
                if($this->session->userdata ('USERID') && $register_user != 1) {
                    $arr['created_by']   = $this->session->userdata ('ADMINTYPE');
                    $arr['created_type'] = $this->session->userdata ('USERID');
                } else {
                ...
                $arr['success'] = 0;
		$this->db->insert('temp_adhi_user', $arr);
		$adhi_user_id   = $this->db->insert_id();
                return $adhi_user_id;
	}
        
        function temp_order($arr){

		$this->db->insert('temp_adhi_orderdetails', $arr);
		return $this->db->insert_id();
	}
        
        function temp_usercourse($arr){
                if($arr['course'] !=''){
                        for($i=0;$i<count($arr['course']);$i++){
                                $d_edition = getDefaultEdition($arr['course'][$i]);
                                $course=array(
                                ...
                                //if($arr['course'][$i] !=5)
                                $this->db->insert('temp_adhi_user_course', $course);
                        }
                } 
		if($arr['subcourse'] !=''){
			$d_edition = getDefaultEdition($arr['subcourse']);
        ...
        function temp_userregistration_update($temp_result, $transaction_id){
            
            if("" != $temp_result){
                $this->db->where(array("temp_id" => $temp_result));
                $this->db->update('temp_adhi_user',array("success" => 1));
            }
            
            if("" != $transaction_id){
                
                $this->db->where("userid", $temp_result);
                $this->db->order_by("temp_id","DESC");
                $query  = $this->db->get('temp_adhi_orderdetails');
                $res = $query->row_array();
                
                if(!empty($res)){
                    $this->db->where(array("temp_id" => $res['temp_id']));
                    $this->db->update('temp_adhi_orderdetails',array("transactionid" => $transaction_id));
                }
                
            }
	}
        
        function renew_reneroll_order($arr){
            $this->db->insert('renew_reenroll_orderdetails', $arr);
            return $this->db->insert_id();
	}
        
        function renew_reneroll_usercourse($arr, $type){
            
                if($arr['course'] !=''){
                    $d_edition = getDefaultEdition($arr['course']);

                    if(0 == $type){
                        $course=array(
                            "org_user_id" => $arr['userid'],
                        ...
                        $course=array(
                            ...
                            "course_id" => $arr['course'],
                            "order_id" => $arr['orderid'],
                            "renew_reenroll_date" => $arr['renew_reenroll_date'],
                            "edition" => $d_edition,
                            "status"  => 1
                        );
                    }
                    $this->db->insert('renew_reenroll_course_details', $course);
                } 
        ...
        function temp_renew_reenroll_update($temp_result, $transaction_id){
            if("" != $temp_result){
                $this->db->where(array("id" => $temp_result));
                $this->db->update('renew_reenroll_orderdetails',array("success" => 1,"transactionid" => $transaction_id));
            }
	}
 }
