...
class User_exam_model extends Model{
	...
	function getquestionid($id,$edition, $where_in_ids = array(), $enable_random_selection = true){
		...
		if(isset ($id) && '' != $id){
            ...
            if($enable_random_selection){
                $random_selection = ' ORDER BY RAND() ';
            }
	...
	*/
	...
	function create_pdf($arr,$unam=''){
				...
				$this->fpdf->Cell(100);
				$this->fpdf->cell(100,10,"9267 Haven Avenue",0,0,'C');
				$this->fpdf->Ln(5);
                                
				$this->fpdf->Cell(100);
				$this->fpdf->SetFont('Arial','B','11');
				$this->fpdf->cell(100,10,"Suite 210",0,0,'C');
				
				$this->fpdf->Ln(5);
				$this->fpdf->Cell(100);				
				$this->fpdf->cell(100,10,"Rancho Cucamonga, CA 91730",0,0,'C');
