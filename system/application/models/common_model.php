...
class Common_model extends Model{
	...
	$ci->email->_smtp_auth     = TRUE; 	    
        ...
        function guest_pass_mail($to_email,$from='',$from_name='', $subject='', $body_content='',$admin='',$attachment = array(),$cc = '', $bcc = '')
	{
                $this->load->library ('email');
                $this->email->_smtp_auth        = $this->config->item('smtp_auth');     
                $this->email->protocol		= $this->config->item('protocol');
...
function get_quiz_status ($id,$course_id) {
	...
	function getCourses($usertype){
		$this->db->select('ac.id,acp.id as acpid, acp.course_type_id,acp.course_id,acp.course_sel_type,acp.amount,ac.course_code,ac.course_name,ac.wieght,acp.books_count');
	...
	function getSupplement($course_id,$edition){
        ...
        function getUserRenewCourses($course_ids, $course_user_type, $userid){
            if(1 == $course_user_type){
                $course_user_type = 2;
            }else if(3 == $course_user_type){
                $course_user_type = 4;
            ...
            $this->db->select("uc.id as usercourse, c.course_name,c.course_code,c.id,cp.amount,c.wieght,c.parent_course_id,c.parent_course_name");
            $this->db->join('adhi_course_price as cp', 'cp.course_id = c.id');
            $this->db->join('adhi_user_course uc', 'uc.courseid = c.id AND uc.userid='.$userid);
            $this->db->where_in('c.id', $course_ids);
            $this->db->where('cp.course_type_id', $course_user_type);
            $query  = $this->db->get('adhi_courses as c');
            return $query->result_array();
        }
