<?php 
class Iframeuser_model extends Model {

  
    function Iframeuser_model()
    {
...
function payment($arr)
	...
	function userregistration($arr){
                if($this->session->userdata('site') == 'associates'){
                    $arr['created_type'] = -4;
                }else if($this->session->userdata('site') == 'alam'){
                    $arr['created_type'] = -3;
                }else{
                    $arr['created_type'] = -1;
                }
...
//echo $ar;
  ...
  require_once $this->config->item ('site_basepath').'system/application/libraries/vbintegration.php';
	...
	function send_mailto_user_old_theme($mail,$order,$trackinfo,$from_admin='',$usertype,$ship='yes'){
		...
		$contents_live		= '';
                ...
                $course_usertype = $this->session->userdata('course_usertype');
	...
	function send_mailto_admin($mail,$order,$admin,$trackinfo, $usertype){
		...
		$subject='Order Details';
                $note_section = ($mail['note'] != '') ? '<tr>
					<td align="left"  width="150" >Note</td>
					<td align="left"  width="150" >'.$mail['note'].' </td>
				</tr>' : '';
		$contents		= '';
		$contents		.= '<b>Account Details</b><br>';
			$contents		.= 
	...
	function send_registration_mail_to_admin($to, $data, $subject){
        ...
        if(end($this->uri->segment_array()) == 'associates'){
            $contents		.= "Adhi Schools Associates Realty";
        }else  if(end($this->uri->segment_array()) == 'alam'){
            $contents		.= "Adhi Schools Alam Realty";
        }else{
            $contents		.= "Adhi Schools Keller Williams";
        }
