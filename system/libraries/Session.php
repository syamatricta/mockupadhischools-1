...
class CI_Session {
	...
	public function sess_create()
	{
		...
		$this->userdata = array(
					...
					'user_data'		=> '',
                                        'user_id'               => 0,
                                        'is_admin'              => 0,
                                        'last_accessed'         => convert_UTC_to_PST_datetime(date('Y-m-d H:i:s'))
				);
	...
	{
                $extra_data = array();
                if ($this->userdata('USERID') != $this->userdata['user_id']){
                    $extra_data['user_id'] = $this->userdata('USERID');
                }
                $is_admin   = ($this->userdata('USERTYPE') == 'A') ? 1 : 0;
                if($is_admin != $this->userdata['is_admin']){
                    $extra_data['is_admin']     = $is_admin;
                }
                $extra_data['last_accessed']    = convert_UTC_to_PST_datetime(date('Y-m-d H:i:s'));
                
                $this->CI->db->where('session_id', $this->userdata['session_id']);
		$this->CI->db->update($this->sess_table_name, $extra_data);
	 ...
	 * @return void
	 */
	 function _get_multi_session($session_id)
	 {
