...
function exam_rule(url){
    ...
    $('poptry').value=1;
    $('rule_form_adhi').action=base_url+"course/exam_rule";
...
function exam_action(url){
	$('examination_form_adhi').action=base_url+'course/examination/n/'+	url;
	$('examination_form_adhi').submit();
	
}
function exam_action_previous(url){
	$('examination_form_adhi').action=base_url+'course/examination/p/'+	url;
	$('examination_form_adhi').submit();
...
function timer(time_rem){
    ...
    offline_timer();
     if(hour=='0' && minute<='0' && second <=1)	{
		...
		//window.location=base_url+'index.php/course/exam_end';
...
function exam_end(url){
	...
	window.location= url+"/course/exam_end";
...
function ajax_update1(){	
	...
	ajaxUpdateTimer = setTimeout("ajax_time_update1()",1000);
}
...
function ajax_time_update1(){
	...
	netTimer = setTimeout("JSfncOffline(0)",10000);
	url	= base_url+'course/ajax_update';
	...
	myAjaxRequest = new Ajax.Request(url,
	                { 
	                    ...
	                    //asynchronous: true,
	                    onFailure: function (){JSfncOffline(1)}
	                }
    );
    
}

function JSfncOffline(change_online_status){
	...
	clearTimeout(netTimer);
	myAjaxRequest.abort();
	if(document.getElementById('divMode') && !is_online)
		document.getElementById('divMode').innerHTML 	= '<font color="red" >Offline</font>';
	if(document.getElementById('hdnMode') && !is_online)
...
function JSfncEndExam(endexm){
	if(endexm==1){
        ...
        if( mode != 0 ){
            $('ad_board').setStyle({'display' : 'block'});
            $('confirm').setStyle({'textAlign' : 'center'});
            document.getElementById('confirm').innerHTML = '<span class="wait_text">Please wait... </span><img src="'+base_url+'images/spinner.gif">';
        }
	  ...
	  if (gCallCount > (60*30)){
	    ...
	    if( mode == 0){
	    	...
	    	///document.getElementById('examviewmain').innerHTML = '<div style="height:50px;color: #FF0000;font-size: 17px;line-height: 25px;padding: 35px;text-align:left;"><img class="oa_img" src=\"'+base_url+'images/warn.png\">The exam is in offline mode due to internet connection unavailability.<br>Please wait, so that we can update your exam scores if internet connection is available within '+disp_left_val+'.</div>';
                if(null == $('message_will_back_online')){
                    document.getElementById('examviewmain').innerHTML = '<div id="message_will_back_online" style="height:50px;color: #FF0000;font-size: 17px;line-height: 25px;padding: 35px;text-align:left;"><img class="oa_img" src=\"'+base_url+'images/warn.png\">The exam is in offline mode due to internet connection unavailability.<br>Please wait, so that we can update your exam scores if internet connection is available within <span id="disp_left_val">'+disp_left_val+'.</span></div><div style="margin-top:30px;text-align:center;"><img src="'+base_url+'images/reskin/ad-board.gif"></div>';
                }
                $('disp_left_val').innerHTML = disp_left_val;
	        ...
	        var exam_jsonString 	= "&exam_tracking_id="+ eval(exam_tracking_id)+"&exam_data="+JSON.stringify(exam_data)+"&is_online="+is_online_num+'&exam_ended='+JSON.stringify({'ended': exam_ended, 'time' : sel_time});
                ...
                adDelayStartAt = Date.now();
                $$('.wait_text').innerHTML = adDelayStartAt;
	        var jsonString 	= JSON.stringify(content);
			var params 		= "jsonarray="+encodeURIComponent(jsonString)+exam_jsonString;
			var url			= base_url+"course/update_score";
...
function JSfncUpdateScore(){
	...
	/*if(1==mode){
		...
		var url			= base_url+"course/update_time";
		new Ajax.Request(url,
	                { 
	                    method		:"post",
...
function fncCheck(obj){
    var delay = (Date.now() - adDelayStartAt) /1000;
    $$('.wait_text').innerHTML = adDelayStartAt+', '+delay;
    
    //10 sec delay for displaying advert in exam window
    if(delay < 12){
        var add_delay   = (12 - delay) * 1000;
        $$('.wait_text').innerHTML = adDelayStartAt+', '+delay+','+add_delay;
        setTimeout(function(){ window.location = base_url+'course/exam_end'; }, add_delay);
    }else{
        window.location = base_url+'course/exam_end';
    }
...
function updateExamDataObj(question_id){	
	...
	if(obj.answer[count]){
		if('' == last_choice){
			var choice_val	= document.getElementById('choice').value;
		}else{
			var choice_val	= last_choice;
		}
...
var offline_interval		= 15*60;//eval(offline_interval_time);//15*60; // in seconds - set in siteconfig.php - 15min
