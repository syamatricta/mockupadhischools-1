...
function fncHandleEvent(type){
	...
	if(isValidTimeRange()==false){
        ...
        if('edit' == type || !($('txtRepeatWeeklyChapters').checked && 'yes' == $('txtRepeatWeeklyChapters').value)){
...
function fncDeleteSuccessiveEvent(masterid){
	var cnfrm = confirm('This operation will delete all related future schedules to successive weeks. Do you want to delete the event(s)?');
	if(cnfrm){
            $('hdnMasterid').value 	= masterid;
            $('txtWhat2Do').value 	= 'delete';
...
function showCapacity(){
	if(15 == $('sltCourses').value){
        ...
        if(5 == $('sltCourses').value){
            $('weeklychapters_checkbox_div').style.display = 'block';            
        }else{
            $('weeklychapters_checkbox_div').style.display = 'none';
        }
}
...
function selectWeeklyChapters(cb){
    if(cb.checked){
        $('chapter_detail_div').style.display = 'none';
        $('repeat_div').style.display = 'none';
        $('txtRepeat').checked  = false;
    }else{
        $('chapter_detail_div').style.display = 'block';
        $('repeat_div').style.display = 'block';
    }
}

function fncCancelEvent(masterid,course_id){
	if(course_id == 5){
            cnfrm = confirm('This operation will postpone all related future schedules to successive weeks. Do you want to continue?');
            if(cnfrm){
                $('hdnMasterid').value 	= masterid;
                $('txtWhat2Do').value 	= 'cancel';
                $('adminscheduleform').submit();
            }else{
                return false;
            }
	}else{
            alert('Only Realestate Priciple Course allowed to do this')
        }
