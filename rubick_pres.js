...
$(document).ready(function(){
	  ...
	  jQuery('#license_info_form').submit(function(event) {
		...
		if(check == 1){
		        ...
		        else
		        {
		            ...
		            jQuery('.valid_msgs').removeClass(remove).hide();
		            //jQuery('.valid_msgs').removeClass(remove).html('Please wait...');
		             
		            //$('#loader_enquiry').show();
                            plsWaitDiv('#contactform', 'show');
		            jQuery.post(base_url+'index.php/home/real_estate_license_info',jQuery('#license_info_form').serialize(), function(resp) {
		                
		            //$('#loader_enquiry').hide()   
                            plsWaitDiv('#contactform', 'hide');
		            
		            
		                if(resp.status !=200){
		                    jQuery('.valid_msgs').removeClass('success_msg').addClass('error_msg').html(resp.msg).show();
		                    jQuery('#math_captcha').val('');
		                }
		                else {
		                    jQuery('#math_captcha,#licencee_name,#licencee_email,#licencee_phone').val('');
		                    jQuery('.valid_msgs').removeClass('error_msg').addClass('success_msg').html(resp.msg).show();
		                }
		               
		            jQuery('#captcha_question').html(resp.math_captcha_question);
		            check = 1;
		            });
		        }
		}
	...
	$('.register a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
            $('#hidcoursetype').val(e.target.innerHTML);
	    show_courses(e.target.innerHTML,$('input[name=package_type]:radio:checked').val());	
            initializeRadio();
	    $('#fixederror').hide();	
	})
	$('.register input[name=package_type]:radio').on('click',function () {
	...
	$('.register').on('click','input[name=course_p]:radio',function (event) {
		event.preventDefault();
	...
	$('.register').on('click','input[name=course_bp]:radio',function (event) {	
		 event.preventDefault()	;
	...
	$('.register').on('show.bs.modal','#geninformation', function (event) {
 	...
 	$('.register').on('click','.mandatory input[type=checkbox]',function (event) { 		 
		 event.preventDefault();
	...
	$('.register').on('click','input[name=course_b]:radio',function (event) {			 
		 event.preventDefault();
                 checkRadio('course_b', $(this).attr('id'), 'iagree');
                 changeAgreeInuptId('iagree');
		 $('#geninformation').modal();
                 $('#geninformation').data('price',$(this).data('price'));
		 $('#geninformation').data('fromid',$(this).attr('id'));
 	...
 	$('#geninformation').on('click','#inoagree_cart',function(){
 		...
 		if( $('#'+radio_check).attr('type')=='checkbox'){
                    ...
                    //var totlal = parseFloat($('#totalweight').val())- parseFloat(cweight);
                    var totlal = parseFloat($('#totalweight').val())- parseFloat($('#'+radio_check).data('courseweight'));
                    $('#totalweight').val(totlal);
		}else{
                    $('#totalweightb').val(0);
		}
	...
	$('.register').on('click','.show_ship_btn',function(event){
            event.preventDefault() 
            ...
            if($('input[name="package_type"]:checked').val()=='Package'){
                if( $("input[name=course_p]").is(":checked")){
                    ...
                    if($('input[name="course_bp"]').length>0 && $('input[name="course_p"]:checked').attr('id')=='course_b'){
                        if( $("input[name=course_bp]").is(":checked")){
                        	checkbox_selection = $('input[name="course_p]');
                        }else{
                            $('#fixederror').show();
                            $('#fixederror').html("Select at least one course") ;
                            return false;
                ...
                if($("input[type=checkbox][name^=course]:checked").length==0 &&  !$("input[name=course_b]").is(":checked")){
                    ...
                    checkbox_selection = $("input[type=checkbox][name^=course]:checked");
				}
            }

			$.each(checkbox_selection, function (key, element){
				course_ids.push($(element).val());
			});
	...
	$('.register').on('click','.show_ship_btn_broker',function(event){
                event.preventDefault() 
	...
	$('.register').on('click','#update_s_zipcode_btn',function(event){ 
		...
		$('#showship').hide();
		$('#shipbutton').show();		 
		$('.register .show_ship_btn').trigger( "click" );
	});
	
	$('.register').on('click','input[name=shiprate_radios]:radio',function (event) { 				
            //unselect all
	...
	$(".staff_box").each(function() {
		...
		var box_height = this_box.outerHeight();
		
		 
		this_box.css("height", box_height);
		this_box.find('.inner').css("height", box_height);
		this_box.css("width", this_box.width());
		this_box.find('.inner').css("width", this_box.width());
		orgbox =  this_box.find('.staff_cnt').outerHeight()

		this_box.hover(function() {			 
			this_box.find(".staff_cnt").stop(true, true).animate({}, 300, "", function() {
  			 this_box.find('.staff_cnt').html(original);
  			 newbox =  this_box.find('.staff_cnt').height()-orgbox;
  			 this_box.css({"height": box_height+newbox,'position':'absolute','z-index':100});
  			 this_box.find('.inner').css("height", box_height+newbox);
  			 this_box.css("width", this_box.width());
			this_box.find('.inner').css("width", this_box.width());
			...
			this_box.find(".staff_cnt").stop(true, true).animate({}, 300, "", function() {
  			 this_box.find('.staff_cnt').html(text);
  			 this_box.css({"height":box_height,'position':'relative','z-index':1});
  			 this_box.find('.inner').css("height", box_height);
  			});
		});
	});
